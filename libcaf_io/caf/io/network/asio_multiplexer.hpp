...
connection_handle conn_hdl_from_socket(T& sock) {
  return
    connection_handle::from_int(int64_from_native_socket(sock.native_handle()));
}

template <class T>
accept_handle accept_hdl_from_socket(T& sock) {
  return
    accept_handle::from_int(int64_from_native_socket(sock.native_handle()));
}
...
class stream {
 public:
  ...
  /**
   * A buffer class providing a compatible interface to @p std::vector.
   */
  ...
  /**
   * Returns the @p multiplexer this stream belongs to.
   */
  ...
  /**
   * Initializes this stream, setting the socket handle to @p fd.
   */
  void init(Socket fd) {
    m_fd = std::move(fd);
  }
  ...
  /**
   * Starts reading data from the socket, forwarding incoming data to @p mgr.
   */
  ...
  /**
   * Configures how much data will be provided for the
   * next @p consume callback.
  ...
  /**
   * Sends the content of the write buffer, calling the @p io_failure
   * member function of @p mgr in case of an error.
   * @warning Must not be called outside the IO multiplexers event loop
   *          once the stream has been started.
   */
  ...
  void read_loop(const manager_ptr& mgr) {
    ...
    switch (m_rd_flag) {
      case receive_policy_flag::exactly:
        ...
        boost::asio::async_read(m_fd,
                                boost::asio::buffer(m_rd_buf, m_rd_size),
                                cb);
        break;
      case receive_policy_flag::at_most:
        ...
        m_fd.async_read_some(boost::asio::buffer(m_rd_buf, m_rd_size),
                             cb);
        break;
...
class acceptor {
 ...
 public:
  /**
   * A manager providing the @p accept member function.
   */
  ...
  /**
   * Returns the @p multiplexer this acceptor belongs to.
   */
  ...
  /**
   * Initializes this acceptor, setting the socket handle to @p fd.
   */
